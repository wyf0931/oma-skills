...
uses the session's configured provider/model, no separate API key needed).
"""
...
def _call_pi(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    # Remove Pi session vars to allow nesting pi -p inside a Pi session.
    env = {
        k: v
    ...
    if result.returncode != 0:
        raise RuntimeError(f"pi -p exited {result.returncode}\nstderr: {result.stderr}")
...
The description appears in Pi's "available_skills" list. When a user sends a query, the agent decides whether to invoke the skill based solely on the title and on this description. Your goal is to write a description that triggers for relevant queries, and doesn't trigger for irrelevant ones.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # SDK path did this as a true multi-turn; `pi -p` is one-shot, so we
...
def main():
    ...
    try:
        ...
        print(f"Error reading eval results {args.eval_results}: {exc}", file=sys.stderr)
        sys.exit(1)
    history = []
    if args.history:
        try:
            ...
            print(f"Error reading history {args.history}: {exc}", file=sys.stderr)
            sys.exit(1)
