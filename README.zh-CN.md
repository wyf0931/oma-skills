...
从真实世界的多源证据中研究并编译一个岗位，产出可部署的 Agent Role Package 与 skill。它把理论、
岗位市场、实践者 know-how、工件和反例分开取证，通过编码与缺口驱动补样形成角色树，再让用户把
公域岗位模型剪枝为自己的组织定义。

```
角色契约 → 证据循环 → 候选岗位模型 → HITL 塑形 → 岗位包 → skill + evals
```

- 在能力可用时用目标和阶段任务跟踪长期研究
- 将独立证据源分派给子代理，但由主代理统一裁决与合成，避免原始研究污染主上下文
- 产出可追溯的职责、权限、边界、交接、工具、记忆和评测定义，而不止是一段人设 prompt
- 用证据置信度与饱和标准判断质量，不把固定资料数量误当成结论

**触发词：** 设计 Agent 岗位、虚拟组织职位、领域专家 skill、岗位边界、能力模型，或希望根据
研究资料构建可证据追溯的岗位模型。

### [content-operations-expert](skills/content-operations-expert/)

围绕已经批准的图文内容，运营小红书、公众号、知乎及后续获批平台的交付系统。它负责平台化包装、
发布就绪检查、发布交接、评论回复运营、反馈/实验闭环与有日期的规则监控，但不吸收选题策略、
原创内容生产或未经授权的账号动作。
